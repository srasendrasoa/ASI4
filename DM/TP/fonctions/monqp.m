...
% A : matrice de taille m x n avec m : nombre de contraintes égalités
% b : vecteur de taille m
% C : vecteur de taille n tq 0 <= x_i <= C_i avec C_i le ieme élément de C
% l : paramètre de stabilisation (joue le même role que lambda dans la
%   : régression logistique)
% verbose : = 1 - on affiche les itérations ; 0 - on affiche rien
% xinit : initialisation de x. Par défaut xinit = zeros(n,1);
% 
% SORTIES
% xnew : solution qui ne contient que les éléments xi non nuls
% lambda : paramètres de lagrange associés à la contrainte égalité A'*x = b
%        : pour un svm de type w'*x + b on a b = lambda
% pos : indique les index des éléments non nuls de x 
%     : çad xnew = x(pos); 
% mu : paramètres de lagrange associés aux contraintes 0 <= x <= C
%
